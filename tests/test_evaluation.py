...
class TestCosyPoseEvaluation():
    ...
    def setUp(self) -> None:
        ...
        for ds_name in self.cfg.ds_names:
            ...
            for detection_type, coarse_estimation_type in self.cfg.detection_coarse_types:
    ...
    def test_config(self):
        ...
        assert self.cfg['detector_run_id'] == 'bop_pbr', "Error: detector_run_id is not correct"
        assert self.cfg['coarse_run_id'] == 'coarse-bop-ycbv-pbr--724183', "Error: coarse_run_id is not correct"
        assert self.cfg['refiner_run_id'] == 'refiner-bop-ycbv-pbr--604090', "Error: refiner_run_id is not correct"
        assert self.cfg['ds_name'] == 'ycbv.bop19', "Error: ds_name is not correct"
        assert self.cfg['inference']['detection_type'] == 'detector', "Error: detection_type is not correct"
        assert self.cfg['inference']['coarse_estimation_type'] == 'SO3_grid', "Error: coarse_estimation_type is not correct"
        assert self.cfg['inference']['SO3_grid_size'] == 576, "Error: SO3_grid_size is not correct"
        assert self.cfg['inference']['n_refiner_iterations'] == 5, "Error: n_refiner_iterations is not correct"
        assert self.cfg['inference']['n_pose_hypotheses'] == 5, "Error: n_pose_hypotheses is not correct"
        assert self.cfg['inference']['run_depth_refiner'] == False, "Error: run_depth_refiner is not correct"
        assert self.cfg['inference']['depth_refiner'] == None, "Error: depth_refiner is not correct"
        assert self.cfg['inference']['bsz_objects'] == 16, "Error: bsz_objects is not correct"
        assert self.cfg['inference']['bsz_images'] == 288, "Error: bsz_images is not correct"
        assert self.cfg['result_id'] == 'ycbv-debug', "Error: result_id is not correct"
        assert self.cfg['n_dataloader_workers'] == 8, "Error: n_dataloader_workers is not correct"
        ...
        assert self.cfg['skip_evaluation'] == True, "Error: skip_evaluation is not correct"
        assert self.cfg['global_batch_size'] == None, "Error: global_batch_size is not correct"
        ...
        assert self.cfg['detection_coarse_types'] == [['detector', 'S03_grid']], "Error: detection_coarse_types is not correct"
    ...
    def test_evaluation_existing_results(self):
        if torch.cuda.is_available():
            ...
            assert bop_eval_cfg.dataset == 'ycbv.bop19.test'
            ...
            pytest.skip("Testing evaluation on CPU only is too time consuming")
