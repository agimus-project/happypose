...
def main():
    ...
    if args.bop_extra_files:
        if args.bop_extra_files == "tless":
            ...
            gdrive_download(
                "cosypose/bop_datasets/ycbv/offsets.txt", BOP_DS_DIR / "ycbv",
            )
            # Evaluation models for YCB-Video (used by other works)
            gdrive_download(
                "cosypose/bop_datasets/ycbv/models_original", BOP_DS_DIR / "ycbv",
            )
            # Keyframe definition
            gdrive_download(
                "cosypose/bop_datasets/ycbv/keyframe.txt", BOP_DS_DIR / "ycbv",
            )
    ...
    if args.ycbv_compat_models:
        gdrive_download(
            "cosypose/bop_datasets/ycbv/models_bop-compat", BOP_DS_DIR / "ycbv",
        )
        gdrive_download(
            "cosypose/bop_datasets/ycbv/models_bop-compat_eval", BOP_DS_DIR / "ycbv",
        )

    if args.cosypose_model:
        gdrive_download(
            f"cosypose/experiments/{args.cosypose_model}",
            LOCAL_DATA_DIR / "experiments",
        )

    if args.megapose_model:
        gdrive_download(
            f"megapose/megapose-models/{args.megapose_model}",
            LOCAL_DATA_DIR / "megapose-models",
        )

    if args.detections:
        gdrive_download(
            f"cosypose/saved_detections/{args.detections}.pkl",
            LOCAL_DATA_DIR / "saved_detections",
        )

    if args.result_id:
        gdrive_download(
            f"cosypose/results/{args.result_id}", LOCAL_DATA_DIR / "results",
        )

    if args.bop_result_id:
        csv_name = args.bop_result_id + ".csv"
        gdrive_download(
            f"cosypose/bop_predictions/{csv_name}", LOCAL_DATA_DIR / "bop_predictions",
        )
        gdrive_download(
            f"cosypose/bop_eval_outputs/{args.bop_result_id}",
    ...
    if args.all_bop20_models:
        ...
        for model_dict in (
            ...
            for model in model_dict.values():
                gdrive_download(
                    f"cosypose/experiments/{model}", LOCAL_DATA_DIR / "experiments",
                )
...
def download_bop_original(ds_name, download_pbr):
    ...
    for suffix in suffixes:
        wget_download_and_extract(
            BOP_SRC + f"{ds_name}_{suffix}.zip", BOP_DS_DIR / ds_name,
        )
