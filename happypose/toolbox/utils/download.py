...
def main():
    ...
    if args.bop_extra_files:
        if args.bop_extra_files == "tless":
            ...
            download("cosypose/bop_datasets/ycbv/offsets.txt", BOP_DS_DIR / "ycbv")
            # Evaluation models for YCB-Video (used by other works)
            download("cosypose/bop_datasets/ycbv/models_original", BOP_DS_DIR / "ycbv")
            # Keyframe definition
            download("cosypose/bop_datasets/ycbv/keyframe.txt", BOP_DS_DIR / "ycbv")

    if args.urdf_models:
        download(f"cosypose/urdfs/{args.urdf_models}", LOCAL_DATA_DIR / "urdfs")

    if args.ycbv_compat_models:
        download("cosypose/bop_datasets/ycbv/models_bop-compat", BOP_DS_DIR / "ycbv")
        download(
