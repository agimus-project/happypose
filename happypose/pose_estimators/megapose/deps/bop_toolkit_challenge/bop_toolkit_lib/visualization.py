...
def draw_rect(im, rect, color=(1.0, 1.0, 1.0)):
    ...
    if im.dtype != np.uint8:
        msg = "The image must be of type uint8."
...
def vis_object_poses(
    ...
    if vis_rgb and rgb is None:
        msg = "RGB visualization triggered but RGB image not provided."
        raise ValueError(msg)

    if (vis_depth_diff or (vis_rgb and vis_rgb_resolve_visib)) and depth is None:
        msg = "Depth visualization triggered but D image not provided."
        raise ValueError(msg)
    ...
    if vis_depth_diff:
        if im_size and im_size != (depth.shape[1], depth.shape[0]):
            msg = "The RGB and D images must have the same size."
            raise ValueError(msg)
        else:
