...
def save_depth(path, im):
    ...
    if path.split(".")[-1].lower() != "png":
        msg = "Only PNG format is currently supported."
...
def load_bop_results(path, version="bop19"):
    ...
    if version == "bop19":
        ...
        with open(path) as f:
            ...
            for line in f:
                ...
                if line_id == 1 and header in line:
                    ...
                    elems = line.split(",")
                    if len(elems) != 7:
                        msg = f"A line does not have 7 comma-sep. elements: {line}"
                        raise ValueError(
                            msg,
                        )
                    ...
                    result = {
                        ...
                        "score": float(elems[3]),
                        "R": np.array(
                            list(map(float, elems[4].split())), float,
                        ).reshape((3, 3)),
                        "t": np.array(
                            list(map(float, elems[5].split())), float,
                        ).reshape((3, 1)),
                        "time": float(elems[6]),
                    }
        ...
        msg = "Unknown version of BOP results."
        raise ValueError(msg)
            ...
            for prop in face_props:
                ...
                if prop[0] == "n_corners":
                    if val != face_n_corners:
                        msg = "Only triangular faces are supported."
                        raise ValueError(msg)
                elif prop[0] == "texcoord":
                    if val != face_n_corners * 2:
                        msg = "Wrong number of UV face coordinates."
                        raise ValueError(msg)
                else:
                    prop_vals[prop[0]] = val
        else:
