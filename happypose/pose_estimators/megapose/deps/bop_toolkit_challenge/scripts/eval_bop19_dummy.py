...
for result_filename in p["result_filenames"]:
    ...
    for est in ests:
        ...
        if est["time"] < 0:
            ...
            if abs(times[result_key] - est["time"]) > 0.001:
                msg = "The running time for scene {} and image {} is not the same for all estimates.".format(est["scene_id"], est["im_id"])
                raise ValueError(
                    msg,
                )
        else:
    ...
    for error in p["errors"]:
        ...
        if subprocess.call(calc_errors_cmd) != 0:
            msg = "Calculation of pose errors failed."
        ...
        for error_sign, error_dir_path in error_dir_paths.items():
            for correct_th in error["correct_th"]:
                ...
                if subprocess.call(calc_scores_cmd) != 0:
                    msg = "Calculation of scores failed."
                    raise RuntimeError(msg)
